...
CREATE INDEX IF NOT EXISTS idx_chunks_tsv_gin ON chunks USING GIN (content_tsv);
...
-- ===========================================================
-- DEV: add user with private documents for each user
-- Users
CREATE TABLE
    IF NOT EXISTS users (
        id SERIAL PRIMARY KEY,
...
-- Quyền truy cập tài liệu (đơn giản: owner) DELETE CASCADE
...
INSERT INTO
    users (id, email, name)
VALUES
    (1, 'test@local', 'Test User') ON CONFLICT (id) DO NOTHING;

-- share multi user's documents (OPT)
